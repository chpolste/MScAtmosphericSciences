...
\startsection[title=Elevation Scanning]
    ...
    Elevation scanning, obtaining brightness temperatures while pointing the
    radiometer away from zenith. Changes weighting functions, more sensitivity
    for the boundary layer leads to better accuracy there \cite[Westwater2004].
    Little impact on higher levels \cite[Cimini2006].

    Horizontal homogeneity becomes increasingly important for flat angles,
    which is problematic the closer one comes to the surface. Likely that
    different air masses are probed \cite[Cimini2006]. Also inhomogeneous
    clouds are problem \cite[Guldner2013]. Scan in two directions and average
    to reduce influence of inhomogeneity. \cite[Cimini2006] noticed a strong
    impact of inhomogeneity problems in neural network retrievals, likely due
    to the non-linearity of the method.

    Requires a high accuracy/sensitivity radiometer, therefore wide bandwidths
    are used \cite[Caddedu2002,Crewell2007].

    Elevation scanning very sensitive to angle misalignment in transparent
    channels \cite[Hewison2006]. One reason why here and in other works, e.g.
    \cite[Massaro2015] or \cite[Crewell2007], only most opaque channels from
    oxygen band are used away from zenith.

    Elevation scans take time and reduce temporal resolution of measurements,
    else trade-offs have to be made wrt to integration time
    \cite[Caddedu2002,Hewison2006].

    Elevation scanning might be useful for rain situation due to less influence
    of rainwater on radiometer \cite[Tan2011,Xu2014], giving better
    signal-to-noise ratio \cite[Cimini2011].

    Some authors have used elevation angles only for lowest layers in
    regression retrievals in order to aviod introduction of noisy predictors
    for upper levels \cite[Chan2010]. Not a problem for optimal estimation.

    Off-zenith observations. Additional challenge for retrieval. RTM has to
    take refraction and Earth's curvature into account. Humidity most easily violates assumption.
    ...
    situation, might contribute to emission \cite[Massaro2015].
