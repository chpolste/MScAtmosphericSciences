...
radiative transfer model is measurement based regression using simultaneous
radiometer observations and radiosonde ascents as training data, but time
series of such data pairs are rare.  The ability to simulate radiometer
measurements provides a way to generate data pairs from radiosonde
climatologies or NWP forecasts. The optimal estimation approach directly
incorporates a numerical radiative transfer model into the retrieval and
additionally makes of of its linearization. The development of microwave
radiative transfer models has therefore been intimately linked to the
development of retrieval techniques.

A few common model are used: MONORTM, MPM, ARTS, .... TODO: refer to comparison
...
\startsection[title={Retrieval Grid},reference={ch:retrievalgrid}]
    ...
    Additional consideration: to which height is profile retrieved? For
    transparent oxygen channels this may have significant effect on brightness
    temperature calculation as tropopause/lower stratosphere have significant
    contributions to radiometer observations.
...
\startsubsection[title={Correcting for Missing Parts of the Profile}]
