...
\startsection[title={State Vector Variables},reference={ch:statevector}]
    ...
    Reducing specific humidity and liquid water content to a combined total
    water content \footnote{Total water content is meant to be specific to
    a unit mass of air similar to specific humidity. For brevity this is not
    explicity written out but always implied throughout this text.} variable is
...
\startsection[title={Characterization of Errors},reference={ch:rtm_errors}]
    ...
    from mathematical simplifications which are often made to speed up
    radiative transfer calculations \cite[alternative=authoryears,left={(e.g.
    }][Lohnert2004].
...
\startsubsection[title=Numerical Considerations]
    ...
    and total water content must therefore be interpolated to the internal
...
\startsubsection[title=Fast Absorption Prediction]
    ...
    be pressure, temperature and the natural logarithm of total water content.
    ...
    the total water content of the state vector into vapor and liquid
...
\startsubsection[title={Correcting for Missing Parts of the Profile},reference=ch:missing_profile]
    ...
    temperatures measured by a radiometer at the ground. With the retrieval
    grid ending at 12 km, the radiative transfer model must account for the
    emission and absorption in the upper atmosphere in order to make accurate
    predictions particularly for the more transparent V band oxygen channels.
    The transparent K band channels are not significantly affected by the upper
    atmosphere because water vapor concentrations are low and emission is weak.
...
\startsubsection[title={Model Intercomparison},reference={ch:model_comparison}]
