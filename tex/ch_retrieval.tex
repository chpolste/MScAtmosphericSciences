...
\startsection[title={Linear Regression},reference=ch:linear_regression]
    ...
    correspond to the individual brightness temperatures, some authors have
    also included quadratic terms to allow the model to better approximate the
    non-linear nature of the retrieval problem. Because the coefficients
    affect the model only linearly they can be determined from a training data
    set in one step without the need for iteration, resulting in a well
    understood, easy and cheap training process.
    ...
    errors and have no guarantee of physical reasonability. This behavior
    limits the transferability of a trained regression model between different
    radiometer sites. (TODO: there have been authors that used data from
    multiple sites, find and cite as exceptions)
...
\startsection[title={Other Retrieval Techniques},reference=ch:otherretrievals]
    ...
    approximate any complex function. Training phase is computationally
    more expensive than that of linear regression but evaluation is again cheap
    once the model parameters have been found. Most neural networks for the
    retrieval of thermodynamic profiles are rather simple, typically with one
    hidden layer and only a single type of activation function. Even though the
    overall performance of neural networks in terms of rmse has not been found
    to be significantly better than that of linear regression, there are
    indications that retrieval performance is better in inversion cases.
    Many other regression models exists but research has been focused on
    linear regression and neural networks so far. Regression models share
    many properties. They are generally easy to use but measures have to be
    taken to avoid overfitting. They often lack methods to estimate uncertainty
    and, as seen for the case of linear regression, even if an uncertainty
    estimate can be derived it might have very limited information content.
    In contrast to optimal estimation retrievals which explicitly use a forward
    model, profiles determined by regression methods need not be consistent
    with the observed brightness temperatures they are retrieved from.
    ...
    correlated and contain less independent pieces of information than the
    number of channels REF. It is also possible to use dimension reduction
    techniques on the atmospheric state or to apply variable transformations.
    The latter will be done in section \in[ch:statevector] in order to enforce
    positivity of retrieved humidity.

    The choice of variables that quantify the atmospheric state is
    ...
    only work with a complete description of the atmospheric state (not true!)
    to ensure accurate radiative transfer calculations. These aspects are one
    of the topics discussed in the following chapter.
...
\startsection[title={Inclusion of Additional Information}]
    ...
    spectrometers, ceilometers, cloud radars or surface in-situ sensors. Most
    radiometers hav built in pressure, humidity and temperature sensors.
    Naturally there is a desire to use information from other instruments to
    improve the retrieval based on radiometer observations.
    ...
    are often included as fixed points into the prior by modifying its
    mean and covariance matrix explicitly (REFS). In-situ measurements of the
    atmospheric state can be directly used to set a value of the prior while
    cloud base heights may be translated to a saturated layer and cloud radar
    observations can be used to explicity prescribe the vertical distribution
    of liquid water. Modification of the prior is generally not trivial and
    care has to be taken not to make the covariance matrix singular. If there
    is no easy way to include additional information into the prior it might be
    sufficient to use them for the first guess of the optimal estimation
    iterative procedure in order to encourage convergence to a profile that is
    consistent with the additional knowledge. A similar effect has the
    use of regularization terms as discussed in section \in[ch:costfunction].
    E.g. profiles known to be cloud-free may be retrieved with a high penalty
    on cloud liquid water (REF?).

    For instruments from which entire atmospheric profiles can be retrieved
    and for which numerical forward models exists, subsequent Bayesian updates
    in the optimal estimation framework are the way to go.
    TODO: As shown by \cite[Rodgers2000] in chapter 4.1.1, there is a more
    ...
    LÖHNERT ET AL have designed a retrieval system that combines cloud radar,
    cloud base radiometer (and more?). Based on subsequent Bayesian updates?

    In Innsbruck research has been focused on the inclusion of in-situ
    measurements from different altitudes as they are available in mountainous
    locations \cite[authoryears][Massaro2013,Meyer2016].

    There is controversy regarding the usefulness of surface observations as
    they are suspected to be influenced to much by inhomogeneous surface layer
    processes and are not representative of the air volume that is responsible
    for the radiometer observations. For humidity retrievals it is common to
    include cloud base height and temperature. Cloud radar my be used to 
