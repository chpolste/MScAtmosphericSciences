...
\startsection[title={Other Retrieval Techniques},reference=ch:otherretrievals]
    ...
    information than the number of channels REF.

    Variable transformations as used also here for humidity, e.g. to enforce
    positivity.

    TODO: disadvantages of regression
    Bayesian linear regression can provide error estimate but its
    interpretation is limited as will be seen shortly. Resulting state is not
    necessarily consistent with brightness temperature measurement (contrast to
    optimal estimation).
...
\startsection[title={Inclusion of Additional Information}]
    ...
    Easier for regression techniques.
    ...
    TODO: Refer to regularization from before.

    Generalization of optimal estimation to Multiple Measurements

    Bayesian approach has natural place for all additional information: prior.
    Multidimensional distributions not that easy to edit directly. Better
    approach: subsequent Bayesian updates. Start with most general prior (e.g.
    climatology) then add additional information using pseudo-forward models.
