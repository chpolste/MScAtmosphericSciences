...
\startsection[title=The Multivariate Gaussian Distribution]
    ...
    The Bayesian framework is based on operations on probability distributions.
    For theoretical derivations it is desireable to work with PDFs for which
    analytic solutions to the necessary Bayesian operations exist. The most
    prominent example of such a distribution is the Gaussian distribution
    ...
    $\VECX, \, \MEANVEC \in \REALS^d$ and $\COVMAT \in \REALS^{d \times d}$
    and the exponential function is applied component-wise. The mean of
    this distribution is $\MEANVEC$ and the mean is also the location of the
    only extremum. This is particularly useful for determination of the mean
    as a simple extremum search directly yields the unique maximum. The
    covariance matrix $\COVMAT$ determines the shape of the distribution. For
    a proper Gaussian distribution, it is always a symmetric positive definite
    matrix. When all off-diagonal elements of $\COVMAT$ are zero, each
    component of the multidimensional distribution is independent from the
    others. If nonzero off-diagonal elements exist, some or all componentes are
    correlated. The two-dimensional example distributions shown in Figure
    \in[fig:bayes_theorem] are all Gaussian distributions. The prior has
    a diagonal covariance matrix and is therefore circular while the likelihood
    function has non-zero off-diagonal elements resulting in tilted,
    ellipsoidal isolines.

    The Gaussian distribution is completely characterized by its mean and
    covariance matrix. In particular, the normalization $((2 \pi)^{d/2}
    \det(\COVMAT)^{1/2})^{-1}$ can be determined solely as a function of
    $\COVMAT$. This is especially convenient in derivations where it is known
    that the result is a Gaussian distribution because the normalization can be
    inferred independently afterwards. This simplification benefits Bayesian
    operations greatly since the marginalization of a Gaussian yields
    a Gaussian and a conditional distribution of two Gaussian variables is
    again Gaussian if the conditioning is linear
    \cite[authoryears][Bishop2006]. With the normalization out of the way it is
    furthermore practical to operate with Gaussian distributions in logarithmic
    space. There, the Gaussian distribution is given by the quadratic form
    ...
    form still contains all information from the original distribution.

    Throughout this thesis it will be assumed again and again a continuous
    variable follows a Gaussian distribution. Its convenient analytic
    properties are a major reason for this choice and it will be tested
    in section \in[ch:statevector] for temperature and humidity. From
    theoretical considerations it is known that the Gaussian distribution is
    the best distribution in terms of information content when only the mean
    and variance of the distribution is known \cite[authoryears][Rodgers2000].
    With the central limit theorem it can be shown that the sum of many sets
    of random variables has a distribution that approaches a Gaussian when
    the number of terms in the sum increases \cite[authoryears][Bishop2006]. In
    practice, measurements are often time averages of observations of
    higher-frequency. The central limit theorem causes such averages to have
    error distributions that are to good approximation Gaussian.
...
\startsubsection[title=Parameter Estimation]
    ...
    \cite[alternative=authoryears,left={(e.g. }][Bishop2006]. and are shown
...
\startsection[title={Optimal Estimation},reference=ch:optimalestimation]
    ...
    $\VECX$. The Bayesian update incorporates a forward model, optimal
    estimation is therefore often called a physical retrieval technique as it
    explicitly makes use of the physics behind radiative transfer
    \cite[alternative=authoryears,left={(e.g. }][Guldner2001,Turner2007].  The
    forward model allows the retrieval of atmoshperic states that are
    consistent with simulations of the radiative transfer while also
    considering the prior information (which is essential to solve the
    non-uniqueness problem of retrieval) and uncertainties associated with the
    components of the method \cite[authoryears][Lohnert2004]. An estimate of
    the error of the retrieved state can also be derived since the Bayesian
    approach propagates the uncertainty estimates of the components. Because of
    the need for error estimates and a forward model, the optimal estimation
    method is more complex than regression techniques and makes higher demands
    on the data used in the retrieval. While being significantly more expensive
    in terms of computational time than regression methods, the computational
    cost has been offset by the hardware development in recent years and
    retrievals in a frequency necessary for operational use are possible with
    a modern desktop computer. Software such as Qpack aims to simplify the
    setup procedure for end users by providing not only a forward model (ARTS)
    but also the routines necessary for the optimal estimation procedure
    \cite[authoryears][Eriksson2005].
    ...
    step into the model analysis. Recently \cite[DeAngelis2016] presented a new
    radiative transfer model providing the capability to assimilate radiometer
    observations directly into an NWP model analysis.
    ...
    this book but only shows the purely Bayesian parts. All distributions
    involved in the optimal estimation procedure are assumed to be Gaussian.
    This allows an analytical derivation and results in a relatively efficient
    retrieval scheme even for atmospheric state vectors of high dimensionality
    where discrete approaches are very costly.
    ...
    \in[ch:construct_prior]. In any case, due to the assumption that the {\PDF}
...
\startsubsection[title={Convergence Testing}]
    ...
    linearization of the forward model this might never happen. Therefore a
    criterion is needed that determines when the state vector has been
    determined with sufficient accuracy. \cite[Rodgers2000] presents multiple
    possibilities which are repeated here.

    One possibility is to determine convergence directly from the value of
    the posterior distribution $-ln(\POSTERIOR)$ whose extemum is determined
    by iterative procedure. The gradient \ineq{gaussminimization} can be
    incorporated in such criteria too \cite[authoryears][Hewison2006]. More
    common are criteria based on the change between to iterations measured in
    observation or atmospheric state space. It is important to scale such
    measures to account for the different magnitudes of the elements of the
    observation or state vector. The error covariance matrices of measured vs.
    ...
    in state space. When these measures are sufficiently small iteration can be
    stopped.

    In practice the issue of convergence is more complicated as not only
    the choice of criterion affects convergence but the minimization technique,
    method of forward model linearization and included radiometer channels
    are all relevant components. The first guess of the iteration procedure
    also influences the convergence properties. Different strategies are
    applied to increase the rate and likelihood of convergence.
    \cite[Cimini2011] found that retrieving temperature first and then humidity
    in a second step improved convergence efficency. \cite[Hewison2006] relaxed
    the chosen criterion after the first 10 iterations to encourage
    convergence. \cite[Turner2014] coupled convergence to a fixed sequence of
    parameters that control the minimization algorithm.
...
\startsubsection[title={1D-VAR, Cost Functions and Regularization},reference=ch:costfunction]
    ...
    The quadratic form minimized in the MAP solution $-ln(\POSTERIOR)$
    ...
    a penalty might be a term involving the lapse rate
    \cite[authoryears][Peckham2000]. \cite[Hewison2006] discussed penalizing
    superadiabatic layers and considered a regularization of excessively high
