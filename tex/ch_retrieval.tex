...
radiation, its measurement is the result of emission and extinction by many
...
techniques for the inversion are applicable. A variety of statistical
...
generally easy to set up if such a data set is available, but they are prone to
...
\cite[Rodgers2000], who treats many aspects of the retrieval problem of
...
\startsection[title={Bayesian Statistics}]
    ...
    denominator acts merely as a normalization and often does not need to be
    ...
    containing a-priori knowledge of $x$. This PDF is called a prior. Then,
    some new data $y$ is observed and $\PROB{y \GIVEN x}$ expresses the
    likelihood of this observation under consideration of the prior knowledge of
    ...
    these methods can work well, but they suffer from the high dimensionality
...
\startsubsection[title=Parameter Estimation]
    ...
    of data $\{\VECX_1, \dots, \VECX_N\}$, whose elements are assumed to be
...
\startsubsection[title={Bayesian Operations for Gaussians},reference=ch:bayes_gauss]
    ...
    this is tedious. Alternatively, it is possible to integrate the
...
\startsection[title={Optimal Estimation},reference=ch:optimalestimation]
    ...
    retrieval, atmospheric states consistent with these error characteristics
    ...
    The optimal estimation method is more complex to set up than regression
    techniques and more expensive in terms of computational time due to the
    requirements of error estimates and a forward model. However, the
    ...
    which provides not only a forward model (ARTS) but also the routines
    ...
    assimilate radiometer observations directly into NWP model fields.
...
\startsubsection[title={Convergence Testing}]
    ...
    the choice of criterion affects convergence. The minimization
    ...
    the criterion to a fixed sequence of parameters that controlled their
...
\startsubsection[title={1D-VAR, Cost Functions and Regularization},reference=ch:costfunction]
    ...
    Bayesian approach when assuming that all variables are Gaussian-distributed.
...
\startsection[title={Linear Regression},reference=ch:linear_regression]
    ...
    other reasonable input data. It is common practice to have a
    ...
    Statistical evaluations of linear regression models used for retrievals
...
\startsection[title={Other Retrieval Techniques},reference=ch:otherretrievals]
    ...
    Individual components of the retrieval can be tweaked next to the method
    ...
    quantify the atmospheric state is of importance too.  While regression
    models are able to retrieve temperature, humidity or liquid water isolated
...
\startsection[title={Priors and Training Data Sets},reference=ch:construct_prior]
    ...
    A possible choice are profiles retrieved by a regression model in
    ...
    Categorization requires that the category can be determined
    ...
    A model can become biased toward a certain meteorological situation if the situation is
    overrepresented in the training data, which has to be considered when collecting
    these \cite[authoryears][Cadeddu2002]. A climatology of radiosonde
...
\startsection[title={Inclusion of Additional Information},reference=ch:additionalinfo]
    ...
    prior has to be set up to accommodate various kinds of such
