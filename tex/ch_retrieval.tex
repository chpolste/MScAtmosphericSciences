...
\startsection[title={Bayesian Statistics}]
    ...
    event or a value that some variable takes. Probability = value between
    0 and 1 representing a continuum of certainty about the state of the
    entity. As an example 0 stands for absolute certainty that a fact is
    false, i.e. impossible, and 1 is absolute certainty that the fact is true.
    All intermediate numbers express certainties between these two extremes.
    ...
    problem the entity is always a continuous variable, e.g. $x \in I$, and its
    probability distribution is a function $I \rightarrow \REALS$ which is
    ...
    conditional $\PROB{x \GIVEN y}$ (read $x$ given $y$). The extension to
    more than two variables is straightforward.

    To derive the fundamental theorem of Bayesian statistics, the product rule
    of probability is needed \cite[authoryears][Bishop2006] which reads
    ...
    determined explicitly. Diachronic interpretation: 
    $\PROB{x \GIVEN y}$ likelihood, $\PROB{y \GIVEN x}$ posterior,
    $\PROB{x}$ prior, $\PROB{y}$ normalization.

    subjective priors, making assumptions explicit, pdf carries uncertainty
    estimates, sampling from pdfs and Monte Carlo, marginalization...
