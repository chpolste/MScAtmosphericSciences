...
\startsection[title={Bayesian Statistics}]
    ...
    conditional $\PROB{x \GIVEN y}$ (read $x$ given $y$). It is possible to remove the
    conditioning of a probability distribution by integrating over all possible
    values of the variable the provides the condition
...
\startsection[title={Optimal Estimation},reference=ch:optimalestimation]
    ...
    \startsubsection[title=Iterative Solutions]
        ...
        It is unnecessary to iterate until the atmospheric state vector has
        beed determined to machine precision and depending on the method of
        linearization of the forward model this might never happen. Therefore
        the iteration is stopped once the difference between $\ITER{\VECX}{i}$
        and $\ITER{\VECX}{i+1}$ is below some chosen threshold. Because
        elements of the state vector may differ greatly in their magnitude they
        should be scaled. An easily available scaling is given by the
        covariance $\ITER{\COVMAT}{i+1}$ calculated during each iteration
        \cite[authoryears][Rodgers2000]. The quadratic form

        \startformula
            (\ITER{\VECX}{i} - \ITER{\VECX}{i+1})^\top \ITER{\COVMATI}{i+1} (\ITER{\VECX}{i} - \ITER{\VECX}{i+1})
        \stopformula

        is scalar, nonnegative (because $\ITER{\COVMAT}{i+1}$ is positive
        definite) and considers the different scales of state vector elements
        appropriately making it a good choice for testing convergence.

        In practical applications, the iterative optimal estimation method
        converges in 75 to 100 \% of cases
        \cite[alternative=authoryears,left={(e.g. }][Cimini2011,Blumberg2015,Martinet2015].  The chosen
        convergence criterion, the included radiometer channels and angles, the
        minimization technique and the method of forward model linearization
        are likely to be major influences on convergence.  Generally cloudy
        cases converge less often than clear-sky cases due to the stronger
        non-linearity of absorption at the cloud threshold
    ...
    \startsubsection[title={Constructing the Prior},reference=ch:construct_prior]
        ...
        Chossing prior faces same challenges as chossing a training data set
        for regression.
        ...
        A-priori knowledge may produce bias, pulling retrievals towards
        ...
        \cite[Chan2010,Tan2011,Massaro2015,Peckam2000])
        or even monthly \cite[Turner2007]. Cloud-free sky can often be
        classified befor retrieval (ceilometer, possibly camera) and incorporated
        into prior/training dataset, e.g.
        \cite[Solheim1998,Lohnert2012,Martinet2015,Xu2015]. Transition between
...
\startsection[title={Linear Regression},reference=ch:linear_regression]
    ...
    usually comes from a radiosonde climatology but NWP output has been used as
    well \cite[authoryears][Guldner2013], while the radiometer measurements are
    often simulated with a numerical radiative transfer model since extensive
    time series of radiometer measurements are rare.
...
\startsection[title=Other Retrieval Techniques]

    Another popular regression model used for the retrieval of thermodynamic
    atmospheric profiles from radiometer measurements is the neural network.
    This is a non-linear model able to approximate even complex functions.
    Due to the non-linearity in its parameters, training the model is
    computationally more expensive than finding the weights in linear
    regression but both are cheap to evaluate once trained. Backpropagation,
    a variant of automatic differentiation, has estabilshed itself as the
    standard technique for efficient, automated training. Studies indicate that
    even a simple network with a single type of activation function and one
    hidden layer of neurons results in better retrieval performance for
    inversion cases than linear regression FIND REFS FOR THIS. There are many
    more regression models aside from linear regression and neural networks
    that have not been tested as retrieval methods yet. The popularity of the
    field of machine learning in the last years has caused a massive increase
    in available regression models.
    ...
    component analysis/empirical orthogonal functions to reduce the
    dimensionality of the retrieval problem. This is possible because
    simultaneous measurements from different radiometer channels are generally
    correlated and contain less independent pieces of
    information than the number of channels REF. The inclusion of additional
    information is a current topic of research. LÖHNERT ET AL have designed
    a retrieval system that combines cloud radar, cloud base radiometer (and
    more?). It is common practice to include surface measurements of
    temperature, humidity and pressure into regression retrievals. These
    measurements as well as measurements of cloud base height and -temperature
    have also been included as fixed points in optimal estimation retrievals (I
    SHOULD HAVE A REF FOR THAT). As mentioned in the introduction
    \cite[Massaro2013] and \cite[Meyer2016] have studied the inclusion of
    measurements from different altitudes as they are available in mountainous
    locations. The choice of variables that quantify the atmospheric state is
    also of importance. While regression techniques are able to retrieve
    temperature, humidity or liquid water alone, optimal estimation techniques
    only work with a complete description of the atmospheric state to ensure
    accurate radiative transfer calculations. These aspects are one of the
    topics discussed in the following chapter.

    TODO: talk about inclusion of surface measurements and that some authors
    think they don't do anything. Mention specialized climatologies combined
    with tries to preclassify profiles.
