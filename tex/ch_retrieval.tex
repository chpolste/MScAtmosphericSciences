...
\startsection[title=The Multivariate Gaussian Distribution]
    ...
    \startsubsection[title={Bayes' Theorem for Gaussians},reference=ret_bayes_gauss]
        ...
        Subscripts are chosen in anticipation of chapter \in[ret_optimest]. The
        posterior has to be Gaussian, therefore
        ...
        \ineq{eq_gaussprodexpand} and \ineq{eq_gausspostexpand} (framed in
        green):
        ...
        \startalign[n=3,align={left,right,left}]
            ...
            \NC \Rightarrow~~ \NC \MEANVECX = \NC
                ...
                + \COVMATA^{-1} \MEANVECA)
                \EQCOMMA \NR[eq_gausspostmean]
        \stopalign
        \stopformula

        where $\COVMATX$ is given by \ineq{eq_gausspostcov}. Equating terms
        linear in $\VECX$ yields the same result.  An alternative derivation of
        \ineq{eq_gausspostcov} and \ineq{eq_gausspostmean} is given by
...
\startsection[title={Optimal Estimation},reference=ret_optimest]
    ...
    \in[ret_bayes_gauss]. First, the forward model has to be linearized.
