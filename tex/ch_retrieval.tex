...
\startsection[title=Other Retrieval Techniques]
    ...
    retrieval that can be tweaked. REF, REF and REF have used principal
    ...
    number of channels REF. The inclusion of additional information is
    a current topic of research. LÖHNERT ET AL have designed a retrieval system
    that combines cloud radar, cloud base radiometer (and more?). It is common
    practice to include surface measurements of temperature, humidity and
    pressure into regression retrievals. These measurements as well as
    measurements of cloud base height and -temperature have also been included
    as fixed points in optimal estimation retrievals (I SHOULD HAVE A REF FOR
    THAT). As mentioned in the introduction \cite[Massaro2013] and
    \cite[Meyer2016] have studied the inclusion of measurements from different
    altitudes as they are available in mountainous locations. The choice of
    variables that quantify the atmospheric state is also of importance. While
    regression techniques are able to retrieve temperature, humidity or liquid
    water alone, optimal estimation techniques only work with a complete
    description of the atmospheric state to ensure accurate radiative transfer
    calculations. These aspects are one of the topics discussed in the
    following chapter.
